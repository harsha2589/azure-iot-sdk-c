...
/* Tests_SRS_IOTHUBTRANSPORTAMQP_01_013: [ `underlying_io_interface` shall be set to NULL. ]*/
/* Tests_SRS_IOTHUBTRANSPORTAMQP_01_014: [ `underlying_io_parameters` shall be set to NULL. ]*/
/* Tests_SRS_IOTHUBTRANSPORTAMQP_07_001: [ If amqp_transport_proxy_options is not NULL getIoTransportProvider shall copy the proxy_hostname, proxy_port, username and password. ] */
TEST_FUNCTION(AMQP_Create_getTLSIOTransport_http_proxy_return_NULL_fail)
{
    // arrange
...
TEST_FUNCTION(AMQP_Create_getTLSIOTransport_http_proxy_seetings)
{
    // arrange
    AMQP_TRANSPORT_PROXY_OPTIONS amqp_proxy_options;
    TRANSPORT_PROVIDER* provider = (TRANSPORT_PROVIDER*)AMQP_Protocol();

...
static IOTHUBMESSAGE_DISPOSITION_RESULT ReceiveMessageCallback(IOTHUB_MESSAGE_HANDLE message, void* userContextCallback)
{
    ...
    if ((contentType = IoTHubMessage_GetCustomContentType(message)) == NULL)
    {
        contentType = "<null>";
    }

    if ((contentEncoding = IoTHubMessage_GetContentEncoding(message)) == NULL)
    {
                ...
                do
                {
                    if (iterator < MESSAGE_COUNT)
                    {
                        ...
                        else
                        {
                            ...
                            (void)IoTHubMessage_SetCustomContentType(messages[iterator].messageHandle, "application/json");
                            (void)IoTHubMessage_SetContentEncoding(messages[iterator].messageHandle, "utf-8");
